function [v,D] = logistpca(x,y,v,show,lambda,eigvalratio)
% [v,D] = logist(x,y,vinit,show,lambda,eigvalratio)
...
if eigvalratio
  ...
  %[V,S] = eig(X*X');
  %V = V(:,find(diag(S)/S(end,end)>eigvalratio));
  %V = fliplr(V);
  %x=X'; clear X;
...
while 1
  ...
  e = (y - mu);
  grad = x'*e; % - lambda .* v;
  inc = pinv(x'*(repmat(w,1,D+1).*x)+diag(lambda)*eye(D+1)) * grad;
